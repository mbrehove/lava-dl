...
from lava.proc.qann.qann import QANN
...
from lava.lib.dl.netx.blocks.process import (
    Input,
    Dense,
    RecurrentDense,
    Conv,
...
class Network(AbstractProcess):
    ...
    def create_input(
        ...
        if len(shape) == 1:
            table_entry = Network._table_str(
                type_str="Input", width=1, height=1, channel=shape[0]
            )
        elif len(shape) == 2:
            table_entry = Network._table_str(
                type_str="Input", width=shape[0], height=shape[1], channel=1
            )
        else:
            table_entry = Network._table_str(
                type_str="Input",
    ...
    def create_dense(
        ...
        if "weight/imag" in layer_config.f:
            ...
            (
                weight_real,
                num_weight_bits_real,
                weight_exponent_real,
                sign_mode_real,
            ) = opt_weights_real
            (
                weight_imag,
                num_weight_bits_imag,
                weight_exponent_imag,
                sign_mode_imag,
            ) = opt_weights_imag
            ...
            if rec:
                ...
                weight_concat, num_weight_bits, weight_exponent, sign_mode = (
                    opt_weight_concat
                )
                ...
                weight, num_weight_bits, weight_exponent, sign_mode = (
                    opt_weights
                )
            ...
            if "delay" in layer_config.keys():
                ...
                if delay.max() > 62:
                    warnings.warn(
                        f"Found max delay of {delay.max()}."
                ...
                if np.isscalar(delay):
                    ...
                    if num_delays == weight.shape[0]:  # axonal delays to syn
                        delay = np.repeat(
                            delay.reshape(-1, 1), weight.shape[1], axis=1
                        )
                    elif num_delays == weight.shape[1]:  # input delays
                        delay = np.repeat(
                            delay.reshape(1, -1), weight.shape[0], axis=0
                        )
                    else:
                        raise RuntimeError(
                            f"Found invalid delay of shape "
